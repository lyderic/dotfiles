...
function main()
	if arg[1] == "--quiet" then quiet = true end
	...
	for _,i in ipairs(state) do
		...
		local c = get_situation(i.binary, quiet)
...
function get_situation(binary, quiet)
...
function deploy(binary, quiet)
	printf("\27[7m %s \27[m", binary)
	if pacman(binary) then return end
	local gz = binary..".gz"
	local curl_call = f("curl %s", quiet and "-s" or "")
	local curl_cmd = f("%s -L -o %q %s/gz/%s", curl_call, "/tmp/"..gz, url, gz)
	print("---> [XeQ] ", curl_cmd)
